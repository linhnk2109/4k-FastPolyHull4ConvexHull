## Threaded 4k-FastPolyHull implementation adapted for use as a library.
## The algorithmic logic is unchanged. As in the sequential source, the
## self-running demo was removed and the optional LoopVectorization.@avx
## annotation was replaced by Base.@inbounds.
##
## Julia must be started with more than one thread for @threads to run in
## parallel. The Python entry points do this through --julia-threads; direct
## Julia calls can use `julia --threads 4 ...`.
using LinearAlgebra
using Random
using Base.Threads
include("utils.jl")

@inline function starting_vertices_parallel_ver1(X,k)
    Is = [[] for _ in 1:(4k+1)]
    len = size(X,1)
...
@inline function find_P_parallel_ver1(X, I,k)
    len = size(X,1)
...
@inline function reduce_1_parallel_ver1(P)
...
@inline function reduce_2_parallel_ver1(P)
    P = sortslices(P, dims = 1, by = x -> x[1])
    Pp = [P[1,:]]
    x_max = P[1,1]
    y_max = P[1,2]
...
@inline function reduce_3_parallel_ver1(P)
    P = sortslices(P, dims = 1, by = x -> x[1])
    Pp = [P[1,:]]
    x_max = P[1,1]
    y_min = P[1,2]
...
@inline function reduce_4_parallel_ver1(P)
    P = sortslices(P, dims = 1, by = x -> -x[1])
    Pp = [P[1,:]]
    x_min = P[1,1]
    y_min = P[1,2]
...
@inline function algorithm_1_parallel_ver1(P)
    l = length(P)
    l<3 && return P
...
@inline function algorithm_2_parallel_ver1(P)
    l = length(P)
    l<3 && return P
    V = Vector{Vector{Float64}}(undef, ceil(Int, l/10))
    sizeV = 1
...
@inline function algorithm_3_parallel_ver1(P)
    l = length(P)
    l<3 && return P
    V = Vector{Vector{Float64}}(undef, ceil(Int, l/10))
    sizeV = 1
...
@inline function algorithm_4_parallel_ver1(P)
    l = length(P)
    l<3 && return P
    V = Vector{Vector{Float64}}(undef, ceil(Int, l/10))
    sizeV = 1
...
@inline function alg_8ksided4CH_parallel_ver1(X,k)
    Is = starting_vertices_parallel_ver1(X,k)
    P = find_P_parallel_ver1(X,Is,k)
    V = [[] for _ in 1:4k]

    @threads for i in 1:4k
        if i >0 && i <=k
            Pp = reduce_1_parallel_ver1(P[i])
            V[i] = algorithm_1_parallel_ver1(Pp)
        elseif i >k && i<=2k
            Pp = reduce_2_parallel_ver1(P[i])
            V[i] = reverse(algorithm_2_parallel_ver1(Pp))
        elseif i >2k && i <=3k
            Pp = reduce_3_parallel_ver1(P[i])
            V[i] = algorithm_3_parallel_ver1(Pp)
        else
            Pp = reduce_4_parallel_ver1(P[i])
            V[i] = reverse(algorithm_4_parallel_ver1(Pp))
        end
    end
    hull = V[1]
    for i in 2:4k
